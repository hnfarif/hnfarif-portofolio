      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
          ...
          <button 
            ...
            className="text-white font-extrabold text-lg tracking-tight select-none flex items-center gap-2 group"
          >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-40 bg-[#030014]/95 backdrop-blur-xl pt-28 px-6 flex flex-col gap-6 md:hidden"
          >
            <div className="flex flex-col gap-5 text-xl font-bold text-gray-300">
              {navLinks.map((link) => (
                <button
                  key={link.target}
                  onClick={() => handleNavClick(link.target)}
