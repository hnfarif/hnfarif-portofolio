    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cvData.skills.map((skillGroup, idx) => (
          <BorderGlow key={idx}>
            <div 
              className="glass-panel p-8 relative overflow-hidden group glass-panel-hover h-full"
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 group-hover:border-violet-500/30 transition-colors duration-300">
              ...
              <div className="flex flex-wrap gap-2">
                {skillGroup.items.map((skill, sIdx) => (
                  <span 
                    key={sIdx} 
                    className="px-3 py-1.5 text-xs font-semibold rounded-full bg-black/5 dark:bg-white/5 text-gray-700 dark:text-gray-300 border border-black/5 dark:border-white/5 hover:border-black/10 dark:hover:border-white/10 hover:text-gray-900 dark:hover:text-white transition-colors duration-200"
                  >
                    {skill}
                  </span>
                ))}
